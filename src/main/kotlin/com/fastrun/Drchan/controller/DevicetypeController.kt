...
import com.fastrun.TempCollection.service.DevicetypeService
...
class DevicetypeController {
    @Resource
    var _service: DevicetypeService? = null

    @GetMapping("get")
    @ResponseBody
    fun get(@RequestParam id: Int): ResponseData {
        val re = ResponseData()
        var result: Devicetype? = _service?.get(id)
    ...
    @PostMapping("create")
    @ResponseBody
    fun create(@RequestBody model: Devicetype): ResponseData {
        val re = ResponseData()
    ...
    @PostMapping("update")
    @ResponseBody
    fun update(@RequestBody model: Devicetype): ResponseData {
        val re = ResponseData()
    ...
    fun list(): ModelAndView {
        var m = ModelAndView()
        m.viewName = "/admin/devicetype/list"
        return m;
    }
