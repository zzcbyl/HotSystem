...
(function () {
    ...
    $(function () {
        ...
        $("#btnAddProduct").on("click", function () {
            ...
            _$modal.find("input[name='ProductNameEN']").val('');
            _$modal.find("input[name='ProductNameKR']").val('');
            //_$modal.find("input[name='Attributes']").val('');
            //_$modal.find("[name='Description']").val('');
            //editorInstances.setEditorValue('Description', '');
            //_$modal.find("input[name='ImgUrl']").val('');
            _$modal.find("#ImgUrlShow").attr('src','');
            _$modal.find("#CategoryID").val('');
            _$modal.find("#KeywordsCN").val('');
            _$modal.find("#KeywordsEN").val('');
            _$modal.find("#KeywordsKR").val('');
            _$modal.find("#HeaderDescriptionCN").val('');
            _$modal.find("#HeaderDescriptionEN").val('');
            _$modal.find("#HeaderDescriptionKR").val('');
        ...
        _$productsTable.jtable({
            ...
            fields: {
                ...
                productNameEN: {
                    ...
                    width: '20%',
                    display: function (data) {
                        return data.record.productNameEN ? data.record.productNameEN : '<div class="middle-in-td">--</div>';
                    }
                },
                productNameKR: {
                    title: '标题（韩文）',
                    width: '20%',
                    display: function (data) {
                        return data.record.productNameKR ? data.record.productNameKR : '<div class="middle-in-td">--</div>';
                    }
                },
                //descriptionCN: {
                //    title: '内容',
                ...
                actions: {
                    ...
                    display: function (data) {
                        ...
                        //编辑
                        if (_permissions.edit) {
                            $('<button class="btn btn-success btn-xs" title="编辑"><i class="fa fa-edit">编辑内容</i></button>')
                                .appendTo($div1)
                                .click(function () {
                                    window.open("/Admin/ProductManage/EditHtml?ID=" + data.record.id, "_blank", "", "");
                                });
                        }
                        $div.appendTo($span);
                        $div1.appendTo($span);
        ...
        function EditProduct(product)
        {
            _$modal.off('show.bs.modal').on('show.bs.modal', function () {
                ...
                _$modal.find("input[name='ProductNameEN']").val(product.productNameEN);
                _$modal.find("input[name='ProductNameKR']").val(product.productNameKR);

                //_$modal.find("input[name='Attributes']").val(product.attributes);
              //  _$modal.find("textarea[name='Description']").text(product.Description);
                //editorInstances.setEditorValue('Description', product.description);
              //  console.log(_$modal.find("textarea[name='Description']").attr("id") + ':' + product.Description);
               
                //_$modal.find("#ImgUrlShow").attr('src',product.imgUrl);
                //_$modal.find("input[name='ImgUrl']").val(product.imgUrl);
                _$modal.find("#CategoryID").val(product.categoryID);

                _$modal.find("#KeywordsCN").val(product.keywordsCN);
                _$modal.find("#KeywordsEN").val(product.keywordsEN);
                _$modal.find("#KeywordsKR").val(product.keywordsKR);
                _$modal.find("#HeaderDescription").val(product.headerDescriptionCN);
                _$modal.find("#HeaderDescription").val(product.headerDescriptionEN);
                _$modal.find("#HeaderDescription").val(product.headerDescriptionKR);
                //_$modal.find("input[name='Sort']").val(product.sort);
...
function hideMenu(menuID) {
    ...
    function showDefaultImageUploadForm() {
        ...
        $("#UploadImageModal").modal("show");
        $("#UploadImageModal #uploadImgBtn").off("click").on("click",function (e) {
            uploadImg(e, function (imgUrl) { $("#ImgUrl").val(imgUrl); $("#ImgUrlShow").attr("src", imgUrl); });
        });
    }
