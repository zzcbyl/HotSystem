(function ($) {
    ...
    LG.ajax = function (options) {
        ...
        //将上面两行代码改成下面一行，edit by sun 20120801
        var url = p.url;
    ...
    LG.appendSearchButtons = function (form, grid) {
        ...
        LG.addSearchButtons(form, grid, container.find("li:eq(0)"), container.find("li:eq(1)"), container.find("li:eq(2)"));
    };

    //创建表单搜索按钮：搜索、高级搜索、清除
    LG.addSearchButtons = function (form, grid, btn1Container, btn2Container, btn3Container) {
        if (!form) return;
        if (btn1Container) {
            LG.createButton({
        ...
        if (btn3Container) {
            LG.createButton({
                ...
                text: '清除',
                click: function () {
                    //选择form中所有not这外的input，设置值为空
                    //吴瑞曾增加 hidden
                    $(":input", form).not(":submit, :reset, :image,:button,:hidden, [disabled]").val("");
                    grid.set('parms', {});
                    grid.loadData();
                }
            });
        ...
        var p = $.extend({
            ...
            success: function (data) {
                ...
                for (var p in data) {
                    ...
                    //针对时间 处理 吴瑞曾修改
                    if (ele.is("input[ltype='date']")) {
                        ele.val(LG.getDateFormatValue(data[p], 'yyyy-MM-dd hh:mm'));
                    }
                    //针对复选框和单选框 处理
    ...
    LG.loadToolbar = function (grid, toolbarBtnItemClick, right, operate) {
        right = right.toLowerCase();
        operate = operate.toLowerCase();
        var items = new Array();
        if (operate.indexOf("notany") <= 0) {
            if (right == "edit") {
                if (operate == "" || operate.indexOf("add") >= 0) {
                ...
                if (operate == "" || operate.indexOf("update") >= 0) {
                    var toolUpdate = {
                        click: toolbarBtnItemClick,
                        text: "修改",
                        img: "../../Lib/icons/silkicons/application_edit.png",
                        id: "modify"
                    };
                    items.push(toolUpdate);
                }
                if (operate == "" || operate.indexOf("delete") >= 0) {
                    var toolDel = {
                        click: toolbarBtnItemClick,
                        text: "删除",
                ...
                if (operate == "" || operate.indexOf("view") >= 0) {
                    var toolView = {
                        click: toolbarBtnItemClick,
                        text: "查看",
                        img: "../../Lib/icons/silkicons/application_view_detail.png",
                        id: "view"
                    };
                    items.push(toolView);
                }
                //加载toolbar  张海波 修改 LG.JS修改 2015-06-25 ↓↓↓↓↓
                if (operate == "" || operate.indexOf("link") >= 0) {
                    var toolLink = {
                        click: toolbarBtnItemClick,
                        text: "添加关联",
                        img: "../../Lib/icons/silkicons/link.png",
                        id: "link"
                    };
                    items.push(toolLink);
                }

                if (operate == "" || operate.indexOf("link_break") >= 0) {
                    var toolBreakLink = {
                        click: toolbarBtnItemClick,
                        text: "取消关联",
                        img: "../../Lib/icons/silkicons/link_break.png",
                        id: "link_break"
                    };
                    items.push(toolBreakLink);
                }
                //加载toolbar  张海波 修改 LG.JS修改 2015-06-25 ↑↑↑↑↑
            }
            else if (right == "view") {
            ...
            if (operate.indexOf("excel") >= 0) {
                var toolExc = {
                    click: toolbarBtnItemClick,
                    text: "导出",
                    img: "../../Lib/icons/silkicons/page_excel.png",
                    id: "excel"
                };
                items.push(toolExc);
            }
    ...
    //关闭Tab项并且刷新父窗口
    LG.closeAndReloadParent = function (tabid, parendTabid) {
        LG.closeCurrentTab(tabid);
        if (tab)
            tab.reload(parendTabid);
    };
    ...
    LG.setGridDoubleClick = function (grid, btnID, btnItemClick) {
        btnItemClick = btnILG.setGridDoubleClick = temClick || toolbarBtnItemClick;
    ...
    //获取日期格式的值
    LG.getDateFormatValue = function (value, format) {
        ...
        if (value instanceof Date) {
            ...
            return LG.getFormatDate(value, format);
        }
        else {
    ...
    //格式化日期
    LG.getFormatDate = function (date, dateformat) {
